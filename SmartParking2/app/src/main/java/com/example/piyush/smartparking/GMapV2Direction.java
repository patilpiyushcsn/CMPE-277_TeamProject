...
public class GMapV2Direction {
    ...
    public Document getDocument(LatLng start, LatLng end, String mode) {
        ...
        Log.d("url", url);
        try {
            HttpConnectionHelper httpConnectionHelper = new HttpConnectionHelper(url, "POST");
            httpConnectionHelper.request_Input(HttpConnectionHelper.DEFAULT_READ_TIME_OUT);
            DocumentBuilder builder = DocumentBuilderFactory.newInstance()
                    .newDocumentBuilder();
            Document doc = builder.parse(httpConnectionHelper.getInStream());
            return doc;
        } catch (Exception e) {
            e.printStackTrace();
        }
